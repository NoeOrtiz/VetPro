...
public class MenuItem extends JPanel {
    ...
    private Icon getIcon() {
        ...
        // Mapea el nombre principal del menú a su respectivo nombre de archivo o número de icono original
        String iconName = switch (menus[0].toUpperCase()) {
            case "HOME" ->
                "0";
            case "USUARIO" ->
                "1";
            ...
            case "HOSPITALIZACION" ->
                "9";
            case "TURNOS DE PELUQUERIA" ->
                "10";
            case "AUDITORÍA", "AUDITORIA" ->
                "11"; // Asegúrate de nombrar tu archivo svg con este número o nombre
            case "BACKUP" ->
                "12";
            case "CONFIGURACION" ->
                "13";
            case "SALIR" ->
                "14";
            default ->
                String.valueOf(menuIndex);
        };

        FlatSVGIcon icon = new FlatSVGIcon("veterinaria/menu/icon/" + iconName + ".svg");
    ...
    public void setSubVisible(int subIndex, boolean visible) {
        // subIndex comienza en 1. El 0 es el botón principal (siempre visible si el item está visible).
        if (subIndex <= 0) {
            return;
        }

        // En el panel principal, los componentes se agregan en el mismo orden que 'menus'
        if (subIndex < getComponentCount()) {
            getComponent(subIndex).setVisible(visible);
        }
        if (popup != null) {
            popup.setSubVisible(subIndex, visible);
        }
        revalidate();
        repaint();
    }
